...
const Page = ({ handleBack }) => {
    ...
    async function handleCodPay() {
        try {
            ...
            console.log("COD payment successful:", response);
    ...
    async function handlePromoCodeApply() {
        try {
            ...
            setPromoError(error.response?.data?.errors || "Unexpected error occurred.");
        }
    }
    ...
    return (
        <Box display="flex" flexDirection="column" alignItems="center" p={2}>
            ...
            <CustomButton title="Cash On Delivery" sx={{ width: '100%', mt: 1 }} onClick={handleCodPay} />
            ...
            <Box sx={{ width: '100%', mt: 4 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', mt: 2 }}>
                    ...
                    />
                    <Button variant="contained" color="primary" onClick={handlePromoCodeApply}>
                        Apply
                    </Button>
                </Box>
                {promoError && <Typography color="error" mt={2}>{promoError}</Typography>}
            </Box>
